...
void CodeInterpreter::interpretFunction(llvm::Function* F, std::shared_ptr<AnalysisStore> FunctionStore) {
  DEBUG_WITH_TYPE(GlobalStore->getLogger()->getDebugType(), GlobalStore->getLogger()->logStartFunction(F));
  if (!FunctionStore)
  ...
  DEBUG_WITH_TYPE(GlobalStore->getLogger()->getDebugType(), GlobalStore->getLogger()->logEndFunction(F));
}
...
void CodeInterpreter::updateSuccessorAnalyzer(std::shared_ptr<CodeAnalyzer> CurrentAnalyzer,
                                              ...
                                              unsigned SuccIdx) {
  llvm::DenseMap<llvm::BasicBlock*, std::shared_ptr<CodeAnalyzer>>& BBAnalyzers = Scopes.back().BBAnalyzers;
...
void CodeInterpreter::interpretCall(std::shared_ptr<CodeAnalyzer> CurAnalyzer, llvm::Instruction* I) {
